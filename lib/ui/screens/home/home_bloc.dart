...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  HomeBloc(
    ...
    _deepLinkRepositorySubscription = _deepLinkRepository.setDeepLinkReceiver((redirectUri) {
  ...
  Future<void> _handleClientInitialized(ClientInitializedEvent event, Emitter<HomeState> emit) async {
  ...
  Future<void> _handleOOBRedirect(OOBRedirectArrivedEvent event, Emitter<HomeState> emit) async {
    _onRedirected(emit, event.redirectUri);
  }

  _onRedirected(Emitter<HomeState> emit, String? uri) async {
  ...
  Future<void> _loadData() async {
    _registeredAccounts = await _registeredAccountsUseCase.execute().catchError((error) {
      _errorHandler.handle(error);
      return Set<Account>.identity();
    });
    _authenticators = await _authenticatorsUseCase.execute().catchError((error) {
      _errorHandler.handle(error);
  ...
  Future<void> _handleReadQrCode(ReadQrCodeEvent event, Emitter<HomeState> emit) async {
  ...
  Future<void> _handleInBandAuthentication(InBandAuthenticationEvent event, Emitter<HomeState> emit) async {
    ...
    if (_registeredAccounts.isEmpty) {
      ...
      final parameter = SelectAccountParameter(
        operationType: OperationType.authentication,
        accounts: _registeredAccounts,
      );
      _globalNavigationManager.pushSelectAccount(parameter);
    }
  }
  ...
  Future<void> _handleChangeDeviceInformation(ChangeDeviceInformationEvent event, Emitter<HomeState> emit) async {
    _globalNavigationManager.pushChangeDeviceInformation();
  }

  Future<void> _handleAuthCloudApiRegistration(AuthCloudApiRegistrationEvent event, Emitter<HomeState> emit) async {
    _globalNavigationManager.pushAuthCloudApiRegistration();
  }

  Future<void> _handleLocalData(LocalDataEvent event, Emitter<HomeState> emit) async {
