...
class AuthenticatorSelectorImpl implements AuthenticatorSelector {
  ...
  void selectAuthenticator(
    ...
    Set<Authenticator> authenticators =
        await whereAsync(context.authenticators, (a) async {
  ...
  Future<bool> _mustDisplay(
    Authenticator authenticator,
    AuthenticatorSelectionContext context,
  ) async {
    final operation =
        _operationTypeRepository.state ?? OperationType.registration;
    ...
    switch (operation) {
      ...
      case OperationType.authCloudApiRegistration:
        return Future.value(
            _mustDisplayForRegistration(authenticator, context));
      case OperationType.authentication:
      case OperationType.deregistration:
        return Future.value(
            isRegistered && authenticator.isSupportedByHardware);
      default:
  ...
  Future<bool> _mustDisplayForRegistration(Authenticator authenticator,
    ...
    if (Platform.isAndroid) {
      ...
      final biometricRegistered = await anyAsync(
          authenticators,
          (Authenticator a) => Future.value(authenticator.aaid.isBiometric &&
              a.registration.isRegistered(username)));
      final canRegisterBiometric =
          await anyAsync(authenticators, (Authenticator a) async {
        return Future.value(
            a.aaid.isBiometric && await context.isPolicyCompliant(a.aaid));
      });
      final canRegisterFingerprint =
          await anyAsync(authenticators, (Authenticator a) async {
        return Future.value(
            a.aaid.isFingerprint && await context.isPolicyCompliant(a.aaid));
      });
