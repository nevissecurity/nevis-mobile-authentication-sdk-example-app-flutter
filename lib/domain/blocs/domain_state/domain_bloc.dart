...
@singleton
class DomainBloc extends Bloc<DomainEvent, DomainState> with BehaviorSubjectMixin<DomainEvent, DomainState> {
  ...
  Future<void> _handleSelectAccount(SelectAccountEvent event, Emitter<DomainState> emit) async {
    emit(DomainSelectAccountState(
  ...
  Future<void> _handleSelectAuthenticator(SelectAuthenticatorEvent event, Emitter<DomainState> emit) async {
    emit(DomainSelectAuthenticatorState(authenticators: event.authenticators, operationType: event.operationType));
  }

  Future<void> _handleTransactionConfirmation(TransactionConfirmationEvent event, Emitter<DomainState> emit) async {
    emit(DomainTransactionConfirmationState(
  ...
  Future<void> _handlePinEnrollment(PinEnrollmentEvent event, Emitter<DomainState> emit) async {
    emit(DomainPinState.enrollment(
      lastRecoverableError: event.lastRecoverableError,
      protectionStatus: event.protectionStatus,
    ));
  }

  Future<void> _handlePinVerification(PinUserVerificationEvent event, Emitter<DomainState> emit) async {
    emit(DomainPinState.verification(
      lastRecoverableError: event.lastRecoverableError,
      protectionStatus: event.protectionStatus,
    ));
  }

  Future<void> _handleFingerPrintVerify(FingerPrintUserVerificationEvent event, Emitter<DomainState> emit) async {
    emit(DomainVerifyFingerPrintState());
  }

  Future<void> _handleBiometricVerify(BiometricUserVerificationEvent event, Emitter<DomainState> emit) async {
    emit(DomainVerifyBiometricState());
  }

  Future<void> _handlePinChange(PinChangeEvent event, Emitter<DomainState> emit) async {
    emit(DomainPinState.pinChange(
      lastRecoverableError: event.lastRecoverableError,
      protectionStatus: event.protectionStatus,
    ));
  }

  Future<void> _handleResult(ResultEvent event, Emitter<DomainState> emit) async {
    _userInteractionOperationStateRepository.reset();
    emit(DomainResultState(description: event.description));
  }

  Future<void> _handleAuthenticationSucceeded(AuthenticationSucceededEvent event, Emitter<DomainState> emit) async {
    if (event.operation == OperationType.deregistration) {
      final username = _userInteractionOperationStateRepository.state?.authenticatorSelectionContext?.account.username;
      emit(DomainAuthenticationSucceededState(
